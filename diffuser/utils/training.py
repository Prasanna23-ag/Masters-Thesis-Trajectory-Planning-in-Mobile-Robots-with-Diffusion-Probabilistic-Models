...
class Trainer(object):
    ...
    def train(self, n_train_steps, writer):
        ...
        return {
            ...
            'loss': float(loss.detach().cpu()), 
            'a0_loss': float(infos.get('a0_loss', torch.tensor(0.0)).detach().cpu()), 
            'a_loss': float(infos.get('a_loss', torch.tensor(0.0)).detach().cpu()), 
            's_loss': float(infos.get('s_loss', torch.tensor(0.0)).detach().cpu()), 
            't_loss': float(infos.get('t_loss', torch.tensor(0.0)).detach().cpu()),
        }
    ...
    def render_reference(self, batch_size=10, step=None):
        """
