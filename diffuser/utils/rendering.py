...
MAZE_BOUNDS = {
    ...
    "custom-test-v0": (0, 100, 0, 100),  
    "birrt-dataset-v0": (0, 4.5, 0, 3.5),
}
...
class Maze2dRenderer(MazeRenderer):
    def __init__(self, env, observation_dim=None):
        ...
        smap_img = imageio.imread(pgm_path)
        mask = smap_img > 0 
        coords = np.argwhere(mask) 
        y0, x0 = coords.min(axis=0)
        y1, x1 = coords.max(axis=0) + 1
        smap_img = smap_img[y0:y1, x0:x1]

        self._background = 255 - smap_img  
        
        
        ###### Map metadata from YAML
        resolution = 0.05 # meters per pixel 
        origin_x, origin_y, _ = [-0.844, -1.35, 0] 
        height_px, width_px = smap_img.shape 
        width_m = width_px * resolution 
        height_m = height_px * resolution
        self._extent = ( origin_x, origin_x + width_m, origin_y + height_m, origin_y )
    ...
    def renders(self, observations, conditions=None, **kwargs):
        ...
        # Convert world → map coordinates (meters)
        mx = x
        my = y

        # Stack into map-frame coordinates
        obs_m = np.stack([my, mx], axis=-1)

        # Render using map-frame coordinates
        return super().renders(obs_m, conditions, **kwargs)
