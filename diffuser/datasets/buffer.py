...
class ReplayBuffer:
    ...
    def add_path(self, path):
        if self._count >= self.max_n_episodes:
            print("[buffer.py] Buffer full — skipping")
            return
        ...
        # Ensure all fields match the observation length
        for key in path:
            if len(path[key]) != path_length:
                print(f"[buffer.py] Truncating {key} from {len(path[key])} to {path_length}")
                path[key] = path[key][:path_length]

        print(f"[buffer.py] Received episode of length {path_length}")


        if path_length > self.max_path_length:
            print(f"[buffer.py] Truncating episode from {path_length} to {self.max_path_length}")
            for key in path:
    ...
    def finalize(self):
        if not hasattr(self, "keys"):
            print("[buffer.py] No episodes added — skipping finalize")
            return
