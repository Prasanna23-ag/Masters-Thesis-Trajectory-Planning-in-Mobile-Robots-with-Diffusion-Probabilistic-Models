...
class DatasetNormalizer:
    ...
    def normalize(self, x, key):
        expected_shape = self.normalizers[key].mins.shape
        if x.shape[-1] < expected_shape[0]:
            pad_width = expected_shape[0] - x.shape[-1]
            x = np.pad(x, (0, pad_width))
        elif x.shape[-1] > expected_shape[0]:
            x = x[:expected_shape[0]]  # truncate if too long
