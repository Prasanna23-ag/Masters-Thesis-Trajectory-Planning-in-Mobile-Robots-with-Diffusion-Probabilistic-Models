...
def sequence_dataset(env, preprocess_fn, load_path=None, dataset=None):
    ...
    # Fallback for single-trajectory datasets
    if dataset['observations'].shape[0] > 0 and np.sum(dataset['timeouts']) <= 1:
        print("[d4rl.py] Detected single trajectory — skipping segmentation")
        yield {k: dataset[k] for k in dataset if 'metadata' not in k}
        return
