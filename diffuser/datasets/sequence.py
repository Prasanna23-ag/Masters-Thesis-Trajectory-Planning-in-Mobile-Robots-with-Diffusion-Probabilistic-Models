...
class SequenceDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        print("[debug] High-level dataset size:", len(self.indices))
    ...
    def make_indices(self, path_lengths, horizon):
        ...
        for i, path_length in enumerate(path_lengths):
            max_start = max(0, min(path_length - 1, self.max_path_length - horizon))
            if not self.use_padding:
                max_start = max(0, min(max_start, path_length - horizon))
